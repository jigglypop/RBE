...
#[test]
fn 기본_경사하강법_성능_테스트() {
    use rbe_llm::core::optimizers::gradient_descent::GradientDescent;
    use std::time::Instant;
    use rand::SeedableRng;

    println!("\n === 기본 경사 하강법(Gradient Descent) 성능 테스트 ===");
    ...
    for epoch in 0..epochs {
        ...
        for i in 0..size {
            for j in 0..size {
                ...
                optimizer.update(&mut packed, i, j, size, size, target, learning_rate);
                total_updates += 1;
            }
        }
        let avg_error = epoch_error / (size * size) as f32;
    ...
    let elapsed = start_time.elapsed();
    let ns_per_op = elapsed.as_nanos() as f64 / total_updates as f64;
    let improvement = ((initial_error - final_error) / initial_error) * 100.0;

    println!("  📈 최종 결과:");
    println!("    - 초기 오차: {:.6}", initial_error);
    println!("    - 최종 오차: {:.6}", final_error);
    println!("    - 개선율: {:.2}%", improvement);
    println!("  ⏱️ 성능:");
    println!("    - 총 소요 시간: {:.2}ms", elapsed.as_millis());
    println!("    - 연산당 속도: {:.1} ns/op", ns_per_op);

    // 검증: 오차가 개선되고, 속도가 합리적인지 확인
    assert!(final_error < initial_error, "오차가 개선되지 않았습니다.");
    assert!(improvement > 10.0, "개선율이 10% 미만입니다.");
    assert!(ns_per_op < 5000.0, "연산 속도가 5000ns를 초과했습니다.");
}
// 
#[test]
fn 기본_경사하강법_수렴_한계_테스트() {
    use rbe_llm::core::optimizers::gradient_descent::GradientDescent;
    use std::time::Instant;
    use rand::SeedableRng;

    println!("\n === 기본 경사 하강법(Gradient Descent) 수렴 한계 테스트 (목표: 0.01 에러) ===");
    ...
    for epoch in 0..max_epochs {
        ...
        if epoch % 100 == 0 {
            println!("  - Epoch {:<5}: 평균 오차 {:.6}, 학습률: {}", epoch, avg_error, learning_rate);
        }
    ...
    // 검증: 최종 오차가 목표치인 0.01 이하인지 확인
    assert!(final_error <= 0.01, "최종 오차가 목표치 0.01을 넘었습니다: {:.6}", final_error);
}
// 
#[test]
fn 모멘텀_옵티마이저_수렴_테스트() {
    use rbe_llm::core::optimizers::momentum::MomentumOptimizer;
    use std::time::Instant;
    use rand::SeedableRng;

    println!("\n === 모멘텀 옵티마이저 수렴 테스트 (목표: 0.01 에러) ===");

    let mut rng = rand::rngs::StdRng::seed_from_u64(1337);
    let mut packed = Packed128::random(&mut rng);
    let mut optimizer = MomentumOptimizer::new(0.9); // beta = 0.9
    ...
    assert!(final_error <= 0.01, "모멘텀 옵티마이저가 목표 오차 0.01에 도달하지 못했습니다: {:.6}", final_error);
}