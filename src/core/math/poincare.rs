...
impl PoincareBallPoint {
    ...
    pub fn to_cartesian(&self) -> (f32, f32) {
        (
            self.r * libm::cosf(self.theta),
            self.r * libm::sinf(self.theta),
        )
    }
...
impl RiemannianGeometry {
    ...
    pub fn exponential_map(
        base: &PoincareBallPoint,
        tangent: &PoincareBallPoint,
    ) -> PoincareBallPoint {
        if tangent.r < 1e-8 {
            return *base;
        }
    ...
    pub fn hyperbolic_distance(p1: &PoincareBallPoint, p2: &PoincareBallPoint) -> f32 {
        // 같은 점인지 확인 (수치적 오차 고려)
        let eps = 1e-6;
        if (p1.r - p2.r).abs() < eps
            && ((p1.theta - p2.theta).abs() < eps || (p1.theta - p2.theta).abs() > 2.0 * PI - eps)
        {
            return 0.0;
        }
