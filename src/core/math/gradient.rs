    ...
    for (batch_idx, target) in targets.iter().enumerate() {
        ...
        for i in 0..rows {
            for j in 0..cols {
                ...
                seed_r_plus.lo =
                    (((r_fp32 + eps).to_bits() as u64) << 32) | theta_fp32.to_bits() as u64;
                ...
                seed_th_plus.lo =
                    ((r_fp32.to_bits() as u64) << 32) | (theta_fp32 + eps).to_bits() as u64;
...
impl AnalyticalGradient for Packed128 {
    ...
    fn analytical_gradient_r(&self, i: usize, j: usize, rows: usize, cols: usize) -> f32 {
        ...
        let sign = if flipped_pattern > original_pattern {
            1.0
        } else {
            -1.0
        };
