use crate::core::math::fused_ops::{
    fused_backward, fused_backward_adam, fused_backward_fast, fused_backward_gemv,
};
...
fn 역전파_성능_비교() {
    ...
    println!(
        "  기존 역전파: {:.2}ms, MSE: {:.6}",
        time_original.as_secs_f64() * 1000.0,
        mse1
    );
    println!(
        "  빠른 역전파: {:.2}ms, MSE: {:.6}",
        time_fast.as_secs_f64() * 1000.0,
        mse2
    );
...
fn 수치적_안정성_테스트() {
    ...
    for (case_idx, target) in extreme_targets.iter().enumerate() {
        ...
        let (mse, rmse) =
            fused_backward_fast(target, &predicted, &mut seed, rows, cols, learning_rate);
        ...
        assert!(
            updated_theta.is_finite(),
            "케이스 {}에서 업데이트된 theta가 무한대: {}",
            case_idx,
            updated_theta
        );

        println!(
            "케이스 {}: MSE={:.6}, RMSE={:.6}, r={:.4}, theta={:.4}",
