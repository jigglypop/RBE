...
fn poincare_점_생성_테스트() {
    ...
    assert!(
        boundary_point.r < 1.0,
        "반지름이 1 미만으로 클램핑되어야 함"
    );
}
...
fn 좌표_변환_테스트() {
    ...
    assert!(
        (converted_back.r - point.r).abs() < 0.01,
        "반지름 역변환 오류"
    );
}
...
fn mobius_덧셈_테스트() {
    ...
    assert!(
        result.r < 1.0,
        "Möbius 덧셈 결과가 푸앵카레 볼 내부에 있어야 함"
    );
