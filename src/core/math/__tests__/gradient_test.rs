...
fn 해석적_미분_성능_벤치마크_테스트() {
    ...
    println!(
        "해석적 미분 vs 수치적 미분 성능 비교 ({}x{} 그리드, {} 반복)",
    ...
    for i in 0..rows {
        for j in 0..cols {
            ...
            if numerical_theta.abs() > 1e-8 {
                let rel_error_theta =
                    ((analytical_theta - numerical_theta) / numerical_theta).abs();
...
fn packed64_성능_테스트() {
    ...
    assert!(
        avg_time_ns < 1000.0,
        "Packed64 compute_weight가 너무 느림: {:.2} ns/호출",
        avg_time_ns
    );
}
