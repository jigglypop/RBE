...
fn 해석적_미분_디버그_상세_분석() {
    ...
    println!(
        "  unclamped: {}, clamped: {}",
        base_pattern_unclamped, base_pattern
    );
    ...
    println!("상태 정보:");
    println!(
        "  primary_hash: {}, primary_state: {}",
        primary_hash, primary_state
    );
    println!(
        "  secondary_state: {}, detail_bits: {}",
        secondary_state, detail_bits
    );
    ...
    println!("변조 팩터:");
    println!(
        "  modulation_factor: {}, detail_factor: {}",
        modulation_factor, detail_factor
    );
    ...
    let primary_value_grad_r = match primary_state {
        ...
        7 => input_angle / (1.0 + scaled_input * scaled_input),
        _ => 0.0,
    };
    println!(
        "primary_value 미분 (상태 {}): {}",
        primary_state, primary_value_grad_r
    );

    // Chain rule 최종 결과
    let abs_primary_grad = if primary_value >= 0.0 { 1.0 } else { -1.0 };
    let final_grad = base_pattern_grad_r * primary_value.abs() * modulation_factor * detail_factor
        + base_pattern
            * abs_primary_grad
            * primary_value_grad_r
            * modulation_factor
            * detail_factor;

    println!("Chain rule 결과: {}", final_grad);
    println!(
        "차이 (chain vs analytical): {}",
        (final_grad - analytical_grad).abs()
    );
}
...
fn 실패한_위치_1_1_분석() {
    ...
    println!(
        "차이 (manual vs analytical): {}",
        (manual_gradient - analytical_grad).abs()
    );
}
...
fn 실패한_위치_3_0_분석() {
    ...
    let computed_value =
        (base_pattern * primary_value.abs() * modulation_factor * detail_factor).clamp(0.0, 1.0);
