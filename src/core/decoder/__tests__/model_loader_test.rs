...
use crate::decoder::{ModelLoader, LoadedWeight};
use crate::encoder::{ModelLayout, WeightInfo};
...
fn 모델_로더_생성_성공_테스트() {
    ...
    // ModelLoader 생성
    let loader = ModelLoader::load_from_directory(temp_path);
    assert!(loader.is_ok());
    
    let loader = loader.unwrap();
    assert_eq!(loader.get_layout().model_name, "test-model");
    assert_eq!(loader.list_weight_names().len(), 2);
}
...
fn 레이아웃_파일_없음_오류_테스트() {
    ...
    // 레이아웃 파일 없이 로더 생성 시도
    let result = ModelLoader::load_from_directory(temp_path);
...
fn 개별_가중치_로드_테스트() {
    ...
    // ModelLoader 생성
    let mut loader = ModelLoader::load_from_directory(temp_path).unwrap();
...
fn 가중치_존재_확인_테스트() {
    ...
    let loader = ModelLoader::load_from_directory(temp_path).unwrap();
...
fn 가중치_복원_및_검증_테스트() {
    ...
    let layout = ModelLayout {
        model_name: "decode-test".to_string(),
        compression_timestamp: "1234567890".to_string(),
        total_layers: 1,
        total_blocks: 1,
        layout_version: "1.0".to_string(),
