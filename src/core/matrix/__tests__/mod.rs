...
fn 전체_워크플로우_테스트() {
    ...
    println!(
        "블록 분할 시간: {:.2} ms",
        partition_time.as_secs_f64() * 1000.0
    );
    ...
    assert!(
        partition_time.as_secs_f64() < 1.0,
        "분할 시간이 너무 오래 걸림"
    );

    println!("[통과] 전체 워크플로우 테스트");
