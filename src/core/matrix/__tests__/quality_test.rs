...
fn 품질등급_기본값_테스트() {
    ...
    for quality in &quality_levels {
        ...
        // 압축률 검증
        assert!(
            compression >= 200.0 && compression <= 2000.0,
            "압축률 범위 오류: {}",
            compression
        );
...
fn 품질통계_디버그_출력_테스트() {
    ...
    assert!(
        debug_str.contains("total_error"),
        "디버그 출력에 total_error 필드 없음"
    );
    assert!(debug_str.contains("psnr"), "디버그 출력에 psnr 필드 없음");
    assert!(
        debug_str.contains("compression_ratio"),
        "디버그 출력에 compression_ratio 필드 없음"
    );
...
fn 품질통계_print_report_테스트() {
    ...
    println!(
        "   압축률 {:.0}:1 → 효율성 등급: {}",
        stats.compression_ratio, efficiency_grade
    );
}
...
fn 품질통계_효율성등급_테스트() {
    ...
    for &(compression_ratio, expected_grade) in &test_cases {
        ...
        assert_eq!(
            efficiency_grade, expected_grade,
            "압축률 {:.0}:1의 효율성 등급이 틀림",
            compression_ratio
        );
        ...
        println!(
            "압축률 {:.0}:1 → 등급 {}",
            compression_ratio, efficiency_grade
        );
    }
