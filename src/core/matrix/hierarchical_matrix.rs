...
use super::{ErrorController, L1Block, L2Block, QualityLevel, QualityStats};
...
impl HierarchicalBlockMatrix {
    ...
    fn create_l1_block(
        ...
        // 압축 오차 계산
        let compression_error = self.compute_compression_error(
            source_matrix,
            &global_params,
            row_start,
            col_start,
        ...
        self.error_controller
            .update_block_error((row_start, col_start), compression_error);

        // L2 블록들 생성 (재귀적 분할)
        let l2_blocks = if self
            .error_controller
            .should_subdivide((row_start, col_start), 1)
        {
    ...
    fn create_l2_blocks(
        ...
        for i in (0..rows).step_by(l2_block_size) {
            ...
            for j in (0..cols).step_by(l2_block_size) {
                ...
                let l2_block = self.create_l2_block(
                    source_matrix,
                    row_start + i,
                    col_start + j,
                    sub_rows,
    ...
    fn create_l2_block(
        ...
        let macro_params =
            self.estimate_local_parameters(source_matrix, row_start, col_start, rows, cols);
            ...
            for l1_block in l1_row {
                ...
                for i in 0..l1_block.rows {
                    for j in 0..l1_block.cols {
                        if l1_block.col_start + j < input.len()
                            && l1_block.row_start + i < output.len()
                        {
                            let weight = l1_block.global_params.fused_forward(
                                i,
                                j,
                                l1_block.rows,
                                l1_block.cols,
                            );
                            output[l1_block.row_start + i] +=
                                weight * input[l1_block.col_start + j];
                        }
                    }
                }
            }
