...
impl UnifiedBackwardPass {
    ...
    pub fn determine_learning_phase(&self) -> DifferentialPhase {
        ...
        if improvement_rate > 0.1 {
            DifferentialPhase::Exploration // 빠른 개선 중
        } else if improvement_rate > 0.01 {
            DifferentialPhase::Exploitation // 점진적 개선
        } else {
            DifferentialPhase::Convergence // 수렴 단계
        }
    }
    ...
    pub fn apply_cached_gradients(&self, packed: &mut Packed128, gradients: (f32, f32), learning_rate: f32) {
        ...
        let new_r = (r_fp32 - learning_rate * gradients.0).clamp(0.1, 2.0);
    ...
    pub fn update_convergence_tracking(&mut self) {
        if self.convergence_tracker.loss_history.len() >= 20 {
            ...
            let older_losses: Vec<f32> = self.convergence_tracker.loss_history
                .iter().rev().skip(10).take(10).cloned().collect();
            ...
            self.convergence_tracker.convergence_rate = if older_avg > 0.0 {
            ...
            // 정체 감지
            if self.convergence_tracker.convergence_rate.abs() < 0.001 {
        ...
        // 히스토리 크기 제한
        if self.convergence_tracker.loss_history.len() > 1000 {
            self.convergence_tracker.loss_history.remove(0);
        }
        if self.convergence_tracker.gradient_history.len() > 1000 {
            self.convergence_tracker.gradient_history.remove(0);
        }
    }
    ...
    pub fn get_convergence_rate(&self) -> f32 {
        self.convergence_tracker.convergence_rate.abs()
    }
