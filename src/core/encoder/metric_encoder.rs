...
impl MetricTensorEncoder {
    ...
    pub fn encode_from_weights(
        ...
        // 고유벡터 인코딩
        let v_t_matrix = svd.v_t.as_ref().ok_or("SVD V matrix not computed")?;
    ...
    pub fn serialize(&self, block: &MetricTensorBlock) -> Result<Vec<u8>, String> {
        ...
        buffer
            .write_u16::<LittleEndian>(block.header.n)
            .map_err(|e| e.to_string())?;
...
impl MetricTensorDecoder {
    ...
    /// 메트릭 텐서 복원 (rank-k 형태)
    pub fn decode_to_rank_k(
        &self,
        block: &MetricTensorBlock,
    ) -> Result<(DMatrix<f32>, DVector<f32>), String> {
    ...
    pub fn compute_inverse(&self, u: &DMatrix<f32>, lambda: &DVector<f32>) -> DMatrix<f32> {
        ...
        // 중간 행렬 역행렬
        let middle_inv = middle.clone().try_inverse().unwrap_or_else(|| {
            eprintln!("Warning: Middle matrix inversion failed, using pseudo-inverse");
            let middle_copy =
                DMatrix::from_iterator(middle.nrows(), middle.ncols(), middle.iter().cloned());
