...
mod tests {
    ...
    use crate::core::encoder::{MetricTensorDecoder, MetricTensorEncoder, RBEEncoder};
    ...
    fn test_메트릭_텐서_vs_RBE_성능_비교() {
        ...
        for (rows, cols) in test_sizes {
            ...
            let rbe_compressed_size = std::mem::size_of_val(&rbe_block)
                + rbe_block.residuals.len()
                    * std::mem::size_of::<crate::packed_params::ResidualCoefficient>();
            ...
            for &rank in &ranks {
                ...
                let metric_block = metric_encoder
                    .encode_from_weights(&weights, rows, cols)
                    .unwrap();
    ...
    fn test_자연_그래디언트_최적화_시뮬레이션() {
        ...
        for iter in 0..num_iterations {
            // 손실 그래디언트 계산 (단순 MSE)
            let euclidean_grad: Vec<f32> = weights
                .iter()
            ...
            // 자연 그래디언트 계산
            let natural_grad = metric_decoder
                .natural_gradient(&metric_block, &euclidean_grad)
                .unwrap();
    ...
    fn test_추론_속도_비교() {
        ...
        println!(
            "  속도 비율: {:.2}x",
            rbe_time.as_secs_f64() / metric_time.as_secs_f64()
        );
    }
}
