...
mod tests {
        ...
        for i in 0..rows {
            for j in 0..cols {
                ...
                let pattern = ((i as f32 / rows as f32) * std::f32::consts::PI).sin()
                    * ((j as f32 / cols as f32) * std::f32::consts::PI).cos()
                    * 0.02;
    ...
    fn test_int_adam_decoding_speed() {
        ...
        println!(
            "64x64 블록 전체 디코딩 시간: {:.2}μs",
            block_decode_time.as_micros()
        );
        println!(
            "픽셀당 평균: {:.0}ns",
            block_decode_time.as_nanos() as f64 / (64.0 * 64.0)
        );

        // 디코딩 속도만 엄격하게 체크
        assert!(
            decode_time_per_pixel.as_nanos() < 500,
            "디코딩이 너무 느립니다"
        );
    }
    ...
    fn test_simd_performance() {
        ...
        let positions: Vec<(usize, usize)> =
            (0..64).flat_map(|r| (0..64).map(move |c| (r, c))).collect();
    ...
    fn test_large_scale_performance() {
        ...
        println!(
            "  총 인코딩 시간: {:.2}초",
            total_encode_time as f64 / 1_000_000.0
        );

        // 목표 달성 확인
        assert!(
            avg_encode_time < 10.0,
            "평균 인코딩 시간이 목표(8μs)를 초과합니다"
        );
    ...
    fn test_int_adam_large_scale_performance() {
        ...
        match result {
            Ok((blocks, time_ms, ratio, rmse)) => {
                ...
                println!(
                    "- 블록당 평균: {:.2}μs",
                    decode_time.as_micros() as f64 / blocks.len() as f64
                );
    ...
    fn test_various_block_sizes() {
        ...
        for (name, block_size, height, width) in test_configs {
            ...
            // 압축
            let result =
                RBEEncoder::compress_with_int_adam(&matrix_data, height, width, block_size, 200);
        ...
        for (layer_name, height, width, block_size) in test_configs {
            ...
            match result {
                Ok((blocks, time_ms, compression_ratio, rmse)) => {
                    ...
                    // 디코딩 통계
                    let avg_decode_time = decode_times.iter().map(|d| d.as_secs_f64()).sum::<f64>()
                        / test_runs as f64;
            ...
            for &block_size in &block_sizes {
                ...
                match result {
                    Ok((blocks, time_ms, ratio, rmse)) => {
                        ...
                        // 몇 개 블록의 잔차 계수 통계
                        let residual_counts: Vec<usize> =
                            blocks.iter().take(10).map(|b| b.residuals.len()).collect();
    ...
    fn test_progressive_k_encoding() {
        ...
        for (data_type, matrix_data) in test_data {
            ...
            // 기본 K=2로 인코딩
            let result =
                RBEEncoder::compress_with_int_adam(&matrix_data, size, size, block_size, 300);
            ...
            if let Ok((blocks, time_ms, ratio, rmse)) = result {
                ...
                println!(
                    "  잔차 계수 분포: 0개={}, 1개={}, 2개={}",
                    residual_histogram[0], residual_histogram[1], residual_histogram[2]
                );
            }
        }
    }
    ...
    fn test_extreme_cases() {
        ...
        println!(
            "4x4 디코딩 정확도: RMSE = {:.2e}",
            calculate_rmse(&tiny_data, &tiny_decoded)
        );
        ...
        println!(
            "직사각형 블록 RMSE: {:.2e}",
            calculate_rmse(&rect_data, &rect_decoded)
        );
        ...
        // 4. 고진폭 데이터
        println!("\n--- 고진폭 데이터 (±10 범위) ---");
        let high_amp_data: Vec<f32> = (0..64 * 64).map(|_| rng.gen_range(-10.0..10.0)).collect();
        ...
        println!(
            "고진폭 데이터 RMSE: {:.2e}",
            calculate_rmse(&high_amp_data, &high_decoded)
        );
    }

    #[test]
    fn test_hippocampal_patch_system() {
        use crate::packed_params::{DeltaPatch, HippocampalMemory, PatchTag};
        ...
        println!(
            "- RMSE 개선율: {:.1}%",
            (1.0 - rmse_patched / rmse_base) * 100.0
        );
        ...
        println!(
            "- 패치 오버헤드: {:.1}%",
            (patch_memory as f64 / core_memory as f64) * 100.0
        );
        ...
        println!(
            "- 기본 디코딩: {:.3} µs/block",
            base_time.as_nanos() as f64 / iterations as f64 / 1000.0
        );
        println!(
            "- 패치 디코딩: {:.3} µs/block",
            patched_time.as_nanos() as f64 / iterations as f64 / 1000.0
        );
        println!(
            "- 속도 오버헤드: {:.1}%",
            ((patched_time.as_nanos() as f64 / base_time.as_nanos() as f64) - 1.0) * 100.0
        );

        // 검증
        assert!(
            rmse_patched < rmse_base,
            "패치 적용 후 RMSE가 개선되어야 함"
        );
        assert!(
            patch_memory < core_memory / 2,
            "패치 크기는 코어의 50% 미만이어야 함"
        );
    }
