...
mod tests {
    ...
    fn test_enhanced_vs_standard_encoding() {
        ...
        for (rows, cols) in test_sizes {
            ...
            let start = Instant::now();
            let enhanced_block =
                encoder.encode_block_int_adam_enhanced(&data, rows, cols, 1000, target_rmse);
            ...
            println!(
                "  - RMSE 개선: {:.1}%",
                (1.0 - enhanced_rmse / standard_rmse) * 100.0
            );
            ...
            // 검증
            assert!(
                enhanced_rmse < target_rmse as f64 * 5.0,
                "향상된 인코딩이 목표 RMSE의 5배 이내여야 함"
            );
            assert!(
                enhanced_ratio >= target_compression_ratio as f64 * 0.5,
    ...
    fn test_adaptive_k_behavior() {
        ...
        for (name, data) in test_cases {
            ...
            for &target_rmse in &target_rmses {
                ...
                // 목표에 근접했는지 확인
                assert!(
                    actual_rmse < target_rmse as f64 * 2.0,
    ...
    fn test_hybrid_vs_standard_optimization() {
        ...
        let standard_encoded =
            encoder.encode_block_int_adam_enhanced(&block_data, 128, 128, 1000, 0.001);
        let standard_time = start.elapsed();

        // 하이브리드 인코딩
        let start = Instant::now();
        let hybrid_encoded =
            encoder.encode_block_hybrid_optimization(&block_data, 128, 128, 1000, 0.001, 1000.0);
        ...
        println!(
            "  - 압축률: {:.0}:1",
            original_size as f64 / standard_compressed as f64
        );
        ...
        println!(
            "  - 압축률: {:.0}:1",
            original_size as f64 / hybrid_compressed as f64
        );
        println!("  - 시간: {:?}", hybrid_time);
        println!(
            "  - RMSE 개선: {:.2}%",
            (1.0 - hybrid_rmse / standard_rmse) * 100.0
        );
    }
    ...
    fn test_hybrid_optimization_speed() {
        ...
        for (rows, cols) in test_sizes {
            ...
            println!(
                "  픽셀당 디코딩: {:.0}ns",
                avg_decode_time.as_nanos() as f64 / (rows * cols) as f64
            );
            println!("  압축률: {:.0}:1", compression_ratio);
            println!("  RMSE: {:.6}", rmse);
    ...
    fn test_hybrid_optimization_speed_limited() {
        ...
        for (rows, cols) in test_sizes {
            ...
            assert!(
                ns_per_pixel < 150.0,
                "디코딩이 너무 느림: {:.0}ns/pixel (목표: <150ns)",
                ns_per_pixel
            );
        }
    }
