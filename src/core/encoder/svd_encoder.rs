...
impl SvdEncoder {
    ...
    pub fn encode(
        ...
        let u_matrix = svd.u.as_ref().ok_or("SVD U matrix not computed")?;
        let v_t_matrix = svd.v_t.as_ref().ok_or("SVD V^T matrix not computed")?;
        ...
        if self.use_f16 {
            for i in 0..rows {
                for j in 0..rank {
                    ...
                    encoded_u
                        .write_u16::<LittleEndian>(f16_bits)
    ...
    pub fn serialize(&self, block: &SvdCompressedBlock) -> Result<Vec<u8>, String> {
        ...
        buffer
            .write_u16::<LittleEndian>(block.header.rows)
            .map_err(|e| e.to_string())?;
        buffer
            .write_u16::<LittleEndian>(block.header.cols)
            .map_err(|e| e.to_string())?;
        buffer
            .write_u16::<LittleEndian>(block.header.rank)
            .map_err(|e| e.to_string())?;
        buffer.push(block.header.flags);

        // 스케일
        buffer
            .write_f32::<LittleEndian>(block.scale)
            .map_err(|e| e.to_string())?;
...
impl SvdDecoder {
    ...
    pub fn deserialize(&self, data: &[u8]) -> Result<SvdCompressedBlock, String> {
        ...
        cursor
            .read_exact(&mut singular_values)
            .map_err(|e| e.to_string())?;
...
mod tests {
    ...
    fn test_svd_encoding_decoding() {
        ...
        println!(
            ...
            &svd.singular_values.as_slice()[..8]
        );
        ...
        for (i, &byte) in block.singular_values.iter().enumerate() {
            ...
            println!(
                "SV[{}]: encoded={}, decoded_norm={:.6}, decoded={:.6}",
