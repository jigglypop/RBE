use crate::core::{
    math::{compute_pixel_value, compute_rbe_basis, compute_rmse, normalize_coords},
...
use omni_wave::{completely_decompose_2d, wavelet as w};
...
impl RBEEncoder {
    ...
    pub fn encode_block(
        ...
        if block_data.len() != rows * cols {
            panic!(
                "block_data 길이({})가 rows * cols({})와 일치하지 않음",
                block_data.len(),
                rows * cols
            );
        }
        ...
        let a_matrix = {
            ...
            if let Some(cached) = cache.get(&cache_key) {
                ...
                // A matrix 생성 (병렬 처리)
                let a_matrix_rows: Vec<RowDVector<f32>> = (0..rows * cols)
                    .into_par_iter()
                    .map(|idx| {
                        let r = idx / cols;
                        let c = idx % cols;
        ...
        let mut sigma_inv = DMatrix::zeros(
            svd.v_t.as_ref().unwrap().nrows(),
            svd.u.as_ref().unwrap().ncols(),
        );

        let tolerance = 1e-10_f32;
        for i in 0..singular_values
            .len()
            .min(sigma_inv.nrows())
            .min(sigma_inv.ncols())
        {
        ...
        let a_pseudo_inv =
            svd.v_t.as_ref().unwrap().transpose() * sigma_inv * svd.u.as_ref().unwrap().transpose();
    ...
    fn encode_single_transform(
        ...
        let mut residual_matrix =
            Array2::from_shape_vec((rows, cols), residual_vector.iter().cloned().collect())
                .unwrap();
        ...
        match transform_type {
            ...
            TransformType::Adaptive => {
                unreachable!("Adaptive transform should be handled separately")
            }
        }
    ...
    pub fn compress_with_config(
        ...
        let coefficients = if let Some(custom_coeffs) = config.custom_coefficients {
            ...
            match config.profile {
                CompressionProfile::UltraHigh => {
                    Self::predict_coefficients_improved(config.block_size) * 4
                } // 2 → 4배
                CompressionProfile::High => {
                    Self::predict_coefficients_improved(config.block_size) * 2
                } // 1 → 2배
                CompressionProfile::Balanced => {
                    Self::predict_coefficients_improved(config.block_size)
                }
                CompressionProfile::Fast => {
                    Self::predict_coefficients_improved(config.block_size) / 2
                } // /4 → /2
                CompressionProfile::UltraFast => {
                    Self::predict_coefficients_improved(config.block_size) / 4
                } // /8 → /4
            }
        };
    ...
    pub fn find_critical_coefficients(
        ...
        while left <= right {
            ...
            match Self::compress_with_profile(
                matrix_data,
                matrix_size,
                matrix_size,
                block_size,
    ...
    pub fn create_quality_encoder(
        ...
        let critical_coeffs = Self::find_critical_coefficients_single_block(
            data,
            rows,
            cols,
            rmse_threshold,
    ...
    pub fn encode_block_int_adam(
        ...
        let data_std = (block_data
            .iter()
            .map(|&x| (x - data_mean).powi(2))
            .sum::<f32>()
            / block_data.len() as f32)
            .sqrt();
        ...
        for step in 1..=steps {
            // 그래디언트 계산
            let grads =
                self.compute_float_gradients(&rbe_params, &residual_coeffs, block_data, rows, cols);
            ...
            if step % 100 == 0 {
                let current_rmse =
                    self.compute_rmse(&rbe_params, &residual_coeffs, block_data, rows, cols);

                // 개선 여부 체크
                if current_rmse < best_rmse * 0.99 {
                    // 1% 이상 개선시
        ...
        // 6. 잔차 계수 포맷팅
        let residual_coefficients: Vec<ResidualCoefficient> = residual_coeffs
            .iter()
            .enumerate()
        ...
        println!(
            "인코딩 완료: {:.1}ms, 수렴 step: {}, 최종 K: {}, 최종 RMSE: {:.2e}",
            encoding_time / 1000.0,
            converged_at_step,
            residual_coefficients.len(),
            best_rmse
        );
    ...
    pub fn compress_with_int_adam(
        ...
        let blocks: Vec<HybridEncodedBlock> = (0..total_blocks)
            ...
            .map(|block_idx| {
                ...
                encoder.encode_block_int_adam_quiet(
                    &block_data,
                    actual_block_height,
                    actual_block_width,
                    steps,
        ...
        let compressed_size: usize = blocks.iter().map(|b| 8 * 4 + b.residuals.len() * 8).sum();
    ...
    pub fn encode_block_int_adam_quiet(
        ...
        for step in 1..=steps {
            ...
            if step % 50 == 0 {
                let rmse = self.compute_rmse_with_k(
                    &rbe_params,
                    &residual_coeffs[..active_k],
                    block_data,
                    rows,
            ...
            if step % 100 == 0 {
                ...
                println!(
                    "  Block({},{}): Step {}: grad={:.2e}, RMSE={:.2e}, K={}, lr={:.6}",
    ...
    fn compute_float_gradients_with_k(
        ...
        for i in 0..8 {
            ...
            let loss_plus =
                self.compute_loss_with_k(&params_plus, residual_coeffs, block_data, rows, cols);
            let loss_minus =
                self.compute_loss_with_k(&params_minus, residual_coeffs, block_data, rows, cols);
        ...
        for j in 0..k {
            ...
            let loss_plus =
                self.compute_loss_with_k(rbe_params, &coeffs_plus, block_data, rows, cols);
            let loss_minus =
                self.compute_loss_with_k(rbe_params, &coeffs_minus, block_data, rows, cols);
                ...
                if coeff.abs() > 1e-8 {
                    for i in 0..rows {
                        for j in 0..cols {
                            let idx = i * cols + j;
                            let freq_contrib =
                                self.compute_frequency_basis(freq_idx, i, j, rows, cols);
        ...
        for i in 0..8 {
            ...
            for r in 0..rows {
                for c in 0..cols {
                    let idx = r * cols + c;
                    let predicted =
                        self.compute_predicted_float(rbe_params, residual_coeffs, r, c, rows, cols);
    ...
    fn compute_frequency_basis(
        ...
        let y_term =
            ((2.0 * i as f32 + 1.0) * u as f32 * std::f32::consts::PI / (2.0 * rows as f32)).cos();
        let x_term =
            ((2.0 * j as f32 + 1.0) * v as f32 * std::f32::consts::PI / (2.0 * cols as f32)).cos();
    ...
    fn compute_extended_gradients(
        ...
        for i in 0..rows {
            for j in 0..cols {
                let idx = i * cols + j;
                let predicted =
                    self.compute_extended_value(basis_params, residual_coeffs, i, j, rows, cols);
                let error = predicted - block_data[idx];
    ...
    pub fn encode_block_int_adam_enhanced(
        ...
        for step in 1..=steps {
            ...
            if step % 25 == 0 && k_increase_cooldown == 0 {
                let rmse = self.compute_enhanced_rmse(
                    &rbe_params,
                    &residual_coeffs[..active_k],
                    block_data,
                    rows,
        ...
        // 최종 RMSE 확인
        let final_rmse = self.compute_enhanced_rmse(
            &rbe_params,
            &residual_coeffs[..active_k],
            block_data,
            rows,
        ...
        println!(
            "Enhanced encoding: RMSE={:.4}, K={} (from {}), ratio={:.0}:1",
            final_rmse,
            active_residuals.len(),
            active_k,
            compression_ratio
        );
    ...
    fn compute_enhanced_gradients(
        ...
        for r in 0..rows {
            for c in 0..cols {
                let idx = r * cols + c;
                let predicted =
                    self.compute_enhanced_prediction(rbe_params, residual_coeffs, r, c, rows, cols);
                let error = predicted - block_data[idx];
    ...
    pub fn encode_block_hybrid_optimization_limited(
        ...
        for step in 0..max_steps {
            ...
            // RMSE 계산
            if step % 50 == 0 {
                let prediction = self.compute_full_prediction(
                    &rbe_params,
                    &residual_coeffs[..active_k],
                    rows,
                    cols,
                ...
                println!(
                    "Step {}: RMSE={:.4}, K={}/{}, ratio={:.0}:1, phase={}",
                    step, rmse, active_k, max_k_allowed, current_ratio, phase_name
                );

                // K 증가 결정 (더 적극적으로)
                if rmse > target_rmse * 1.5
                    && active_k < max_k_allowed
                    && current_ratio > target_compression_ratio * 0.8
                {
                    let rmse_ratio = rmse / target_rmse;
                    let old_k = active_k;
                    ...
                    let current_residual = self.compute_residual_error(
                        &rbe_params,
                        &residual_coeffs[..old_k],
                        block_data,
                        rows,
                        cols,
                    );
        ...
        let final_prediction =
            self.compute_full_prediction(&rbe_params, &residual_coeffs[..active_k], rows, cols);
    ...
    fn initialize_rbe_params(&self, block_data: &[f32], rows: usize, cols: usize) -> [f32; 8] {
        ...
        let variance =
            block_data.iter().map(|&x| (x - mean).powi(2)).sum::<f32>() / block_data.len() as f32;
        ...
        for i in 0..rows.min(8) {
            for j in 0..cols.min(8) {
                ...
                for k in 0..4 {
                    ...
                    freq_energy[k] += value
                        * (freq * i as f32 * PI / rows as f32).cos()
    ...
    pub fn encode_vector_poincare(&mut self, vector_data: &[f32]) -> HybridEncodedBlock {
        ...
        let variance = vector_data.iter().map(|&x| (x - mean).powi(2)).sum::<f32>() / size as f32;
        let std_dev = variance.sqrt();

        // 2. 데이터를 푸앵카레 볼로 매핑
        // 정규화된 데이터를 [0, 1) 범위로 변환
        let normalized: Vec<f32> = vector_data
            .iter()
            .map(|&x| {
                if std_dev > 1e-6 {
        ...
        for step in 1..=100 {
            ...
            // 그래디언트 정규화
            let n = size as f32;
            for g in &mut grads_rbe {
                *g /= n;
            }
            for g in &mut grads_res {
                *g /= n;
            }
        ...
        // 5. 유의미한 잔차만 저장
        let residuals: Vec<ResidualCoefficient> = residual_coeffs
            .iter()
            .enumerate()
    ...
    pub fn encode_vector_simple(&mut self, vector_data: &[f32]) -> HybridEncodedBlock {
        ...
        let residuals = if self.config.enable_residual {
            ...
            for i in 0..size {
                let x = if size > 1 {
                    (i as f32 / (size - 1) as f32) * 2.0 - 1.0
                } else {
                    0.0
                };
